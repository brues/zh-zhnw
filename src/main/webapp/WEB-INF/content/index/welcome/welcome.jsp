...
<head>
    ...
    <script>
        function goxiangmurenyuan(id){
            $("#hiddenFormId").val(id);
            $("#hiddenForm").attr("action","${pageContext.request.contextPath}/project/hour");
            $("#hiddenForm").submit();
        }
    </script>
...
<div class="container" style="width: 100%;max-width:95%;height: 100%;padding-top: 30px;">
    <div class="container-fluid" >
        <div class="row-fluid">
            <div class="span12">
                <span style="font-size: 15pt;color: red;">未付款合同信息</span><br/><br/>
                <form id="pageAction" action="${pageContext.request.contextPath}/welcome/project" method="post" style="font-size: 10pt;">
                    <div>
                        ...
                        <select id="selectDaysSou" name="days" class="selectpicker bla bla bli"  data-live-search="true">
                            <c:forEach var="index" begin="1" end="10" step="1">
                                <c:choose>
                                    <c:when test="${index==days}">
                                        <option selected="selected" value="${index}">${index}天内</option>
                                    </c:when>
                                    <c:otherwise>
                                        <option value="${index}">${index}天内</option>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
                        </select>
